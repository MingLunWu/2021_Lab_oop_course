class Tokenizer:
    ...
    def encode(self, text):
        result = list()
        for word in text:
            result.append(self.dictionary.get(word, 8)) # 8 is unknown.
        return result
    
    def decode(self, indexes):
        result = list()
        reverse_dictionary = {v:k for (k,v) in self.dictionary.items()} # List Comprehension
        for idx in indexes:
            result.append(reverse_dictionary.get(idx, '[UNK]'))
        return result
